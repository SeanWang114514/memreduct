...
// ---------------------------------------------------------------------------
// GUID -> uID fallback (device compatibility fix)
//
// Some shells (observed: Windows 10 19045 ARM64 on Snapdragon tablets)
// reject NIF_GUID-based tray icons: Shell_NotifyIconW(NIM_ADD) fails with
// E_FAIL and the icon never shows in the notification area. When that is
// detected at tray creation, the icon is registered with the classic uID
// scheme instead, and every later tray operation for that hwnd uses the
// same uID. Normal systems keep the GUID path untouched.
// ---------------------------------------------------------------------------

#define R_TRAY_UID_MAX 4

typedef struct _R_TRAY_UID_ENTRY {
	HWND hwnd;
	UINT uid;
} R_TRAY_UID_ENTRY;

static R_TRAY_UID_ENTRY tray_uid_table[R_TRAY_UID_MAX] = {0};

static VOID _r_tray_uid_set (
	_In_ HWND hwnd,
	_In_ UINT uid
)
{
	for (ULONG i = 0; i < R_TRAY_UID_MAX; i++)
	{
		if (tray_uid_table[i].hwnd == hwnd)
		{
			tray_uid_table[i].uid = uid;
			return;
		}
	}
	...
	for (ULONG i = 0; i < R_TRAY_UID_MAX; i++)
	{
		if (!tray_uid_table[i].hwnd)
		{
			tray_uid_table[i].hwnd = hwnd;
			tray_uid_table[i].uid = uid;
			return;
		}
	}
...
)
{
	for (ULONG i = 0; i < R_TRAY_UID_MAX; i++)
	{
		if (tray_uid_table[i].hwnd == hwnd)
		{
			tray_uid_table[i].hwnd = NULL;
			tray_uid_table[i].uid = 0;
			return;
		}
	}
}

static UINT _r_tray_uid_get (
	_In_ HWND hwnd
)
{
	for (ULONG i = 0; i < R_TRAY_UID_MAX; i++)
	{
		if (tray_uid_table[i].hwnd == hwnd)
			return tray_uid_table[i].uid;
	}

	return 0;
}

static VOID _r_tray_applyuid (
	_Inout_ PNOTIFYICONDATA nid,
	_In_ HWND hwnd
)
{
	UINT uid = _r_tray_uid_get (hwnd);
...
)
{
	...
	_r_tray_complete (&nid, hicon, tooltip, is_hidden);

	is_ok = Shell_NotifyIconW (NIM_ADD, &nid);

	if (!is_ok && !uid)
	{
		// GUID-based icons are rejected by some shells (Windows 10 ARM64
		// tablet builds). Fall back to the classic uID scheme.
		uid = 1;

		_r_tray_uid_set (hwnd, uid);
